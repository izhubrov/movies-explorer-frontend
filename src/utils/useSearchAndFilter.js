...
export default function useSearchAndFilter({moviesItems,savedMovies,setIsFinishSavedSearching,setErrorMoviesServer}) {
  ...
  function handleSearchMovies() {
    ...
    if (isShortMoviesFilterOn && searchedMovies && searchedMovies.length !==0) {
      const filteredSearchedMovies = handleDurationFilter(searchedMovies);
      setSearchedMoviesItems(filteredSearchedMovies);
    } else {
      searchInputValue && localStorage.setItem("searchedMovies", JSON.stringify(searchedMovies));
  ...
  function handleSearchSavedMovies() {
    ...
    if (isShortSavedMoviesFilterOn) {
      const filteredSearchedSavedMovies = handleDurationFilter(searchedSavedMovies);
